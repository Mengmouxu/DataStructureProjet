...
from utils.Get_config import get_packages,get_packages_queue,get_packages4,write_yaml, clear_yaml, read_yaml_all
...
class packageLog(package):
    ...
    def add_Log(self, time, location, event):
        ...
        event = 1 -> PROCESSING \\
        event = 2 -> SENT \\
        events = ["ARRIVED", "PROCESSING", "SENT", "WAITING"]
        """
        events = ["ARRIVED", "PROCESSING", "SENT", "WAITING"]
        event_log = {
            "Time": time,
...
def Init_packageLog_queue(Name_Ind):
    """
    Init the packageLog list with all packages at the beginning of simulation.
    """
    packages = get_packages_queue()
    PL = []
    for i in range(len(packages)):
        pl = packageLog(i, Name_Ind, packages)
        PL.append(pl)
...
def Init_packageLog4(Name_Ind):
    """
    Init the packageLog list with all packages at the beginning of simulation.
    """
    packages = get_packages4()
    PL = []
    for i in range(len(packages)):
        pl = packageLog(i, Name_Ind, packages)
        PL.append(pl)
